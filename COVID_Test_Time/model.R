#### COVID Prediction Version 4
####
### Changes from Version 3:
### Added an argument: DynamicShock
### Default value FALSE
### If DynamicShock = TRUE, the number of imported new infections is proportional to
...
covidpred <- function(n, nUNP, nVAXP, nEVP, nCVP, AUNP0, AVAXP0, AEVP0, ACVP0,
                      ...
                      freqShock, DynamicShock=FALSE, Xshock, testfreq_UNP, testfreq_VAXP, testfreq_EVP, testfreq_CVP,
                      Se, Sp)
{
  UUNP0 <- nUNP - AUNP0
  EUNP0 <- 0
  ...
  ### antibody-protection adjusted total number of individuals in the A states
  Atot <- N
  ...
  for (i in 1:ncycles)
  {
    ...
    if (DynamicShock == FALSE)
      X[i+1] <- It[i+1] * Xshock * sumUt_wt/sumUt
    else X[i+1] <- It[i+1] * Xshock * (sumUt_wt/sumUt) * (Atot[i]/Atot[1])
    #if (i < ncycles) print(c(i+1, X[i+1], Atot[i]))
