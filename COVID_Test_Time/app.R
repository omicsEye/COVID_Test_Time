...
source("user_parameters.R", local = T)
...
ui <- fluidPage( # App title ----
                ...
                pageWithSidebar(
                  ...
                  mainPanel(# Output: Histogram ----
                            tabsetPanel(
                              tabPanel(
                                "Cumulative on isolation",
                                plotlyOutput(outputId = "accumlative")
                              ),
...
server <- function(session, input, output) {
  ...
  output$newInf <- renderPlotly({
    results <-  do.call(covidpred, params())
    results[, day := .I]
    p <- ggplot2::ggplot(results, aes(x = day, y = newinf)) + # *results$n/100)) +  # i think newinf is actual number of new infections
  ...
  output$results <- DT::renderDataTable({
    results <-  do.call(covidpred, params())
    DT::datatable(results, options = list(
      lengthMenu = c(10, 30, params()$ncycles),
      pageLength = 5
    ))
  })
  ...
  output$customplot <- renderPlotly({
    x = seq(
      from = input$lower,
      to = input$upper,
      by = (input$upper - input$lower) / input$steps
    )
    dt <- data.table(x)
    fun = input$fun
    for (i in x) {
    ...
    dt[, xfield := paste0(
      input$xfield,
      " = ",
      round(get(input$xfield), 1),
      '\n',
      fun,
      ' of ',
      input$yfield,
      " = ",
      round(get(input$yfield), 1)
    )]
    ggplotly(
      ggplot(dt, aes(
        get(input$xfield), get(input$yfield), label = xfield
      )) + geom_line(
        method = "glm",
        size = .5,
        color = 'black',
